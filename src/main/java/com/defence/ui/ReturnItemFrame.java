...
/**
 * ReturnItemFrame — Return equipment using an item picker dropdown.
 */
public class ReturnItemFrame extends JFrame {
    ...
    public ReturnItemFrame() {
        setTitle("Defence Logistics — Return Item");
        setSize(540, 400);
    ...
    private void initializeUI() {
        ...
        JLabel title = new JLabel("     📥 RETURN EQUIPMENT");
    ...
    private void showItemDetails() {
        ...
        if (item != null) {
            itemInfoLabel.setText("📦 Current Stock: " + item.getQuantity()
                + "  |  " + item.getCategory());
        }
    }
    ...
    private int getSelectedItemId() {
        ...
        try { return Integer.parseInt(selected.split(" — ")[0].trim()); }
    ...
    private void returnItem() {
        ...
        try { qty = Integer.parseInt(qtyStr); }
            ...
            JOptionPane.showMessageDialog(this,
                "Quantity must be a number!", "Validation", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (qty <= 0) {
            JOptionPane.showMessageDialog(this,
                "Quantity must be positive!", "Validation", JOptionPane.WARNING_MESSAGE);
            return;
        }

        boolean success = invManager.returnItem(itemId, qty);
        if (success) {
            txManager.recordTransaction(new Transaction(itemId, "RETURN", qty,
                    SessionManager.getInstance().getUsername()));
            JOptionPane.showMessageDialog(this,
                "✅ Item returned successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
            refreshCombo();
        } else {
            JOptionPane.showMessageDialog(this,
                "❌ Return failed!", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
